...
export default function CleanroomSite({base='',marketing=false,themeControl}:{base?:string;marketing?:boolean;themeControl?:ReactNode}){
  ...
  return <div className="cr-site" lang={locale}>
    ...
    <main id="main">
      <header className="cr-hero"><span className="cr-index">A NOTEBOOK FOR MAKING THINGS TOGETHER</span><Image unoptimized className="cr-wordmark" src={asset('logo.svg')} width="550" height="135" alt="cleanroom"/><h1>{l.tag}</h1><p>{l.intro}</p><div className="cr-hero-actions"><a className="cr-primary" href={marketing?'#install':'#play'}>{marketing?l.start:(ja?'操作を試す':'Try the workspace')}</a><a href={marketing?'https://ag3497120.github.io/cleanroom/':'https://verantyx.ai'}>{marketing?(ja?'ブラウザで体験':'Try in your browser'):(ja?'プロジェクトについて':'About the project')} ↗</a></div><span className="cr-edition">SOURCE PREVIEW · YOUR DECISIONS, YOUR PACE</span></header>
      {marketing&&install}
      ...
      {!marketing&&install}
      <section className="cr-film" id="work"><div className="cr-film-media"><video controls playsInline preload="none" poster={asset('cli-demo-poster.png')} aria-label="Recorded real CLI with scripted demo data"><source src={asset('cli-demo.mp4')} type="video/mp4"/><track kind="captions" src={asset('cli-demo.'+locale+'.vtt')} srcLang={locale} label={l.name} default/><a href={asset('cli-demo.gif')}>View GIF</a></video><small>{ja?'実際のCLIをプログラムで操作・録画。AI出力はデモ用データ。':'Programmed recording of the real CLI. AI outputs are fixture data.'}</small></div><div><span className="cr-index">02 / WHAT IS CLEANROOM?</span><h2>{l.known}</h2><p>{l.meaning}</p><ol>{l.items.map((x,i)=><li key={x}><span>0{i+1}</span>{x}</li>)}</ol><a href={guide}>{l.read} ↗</a></div></section>
      ...
      <section className="cr-origin"><span className="cr-index">04 / WHY I STARTED</span><h2>{ja?'作る経験まで、手放したくなかった。':'I did not want to lose the experience of making.'}</h2><p>{l.origin}</p><div><a href={REPO+'/blob/main/core/docs/origins/README.md'}>{ja?'原文と開発記':'Sources & development notes'} ↗</a></div></section>
    ...
    <footer className="cr-site-footer"><Link className="cr-small-brand" href={base+'/'}><Image unoptimized src={asset('mark.svg')} alt="" width="32" height="32"/>cleanroom</Link><p>{l.foot}</p><a href={REPO}>GitHub</a>{marketing&&<Link href="/apps/">Other projects</Link>}</footer>
  </div>;
}
