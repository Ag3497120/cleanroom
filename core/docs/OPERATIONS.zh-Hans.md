...
新安装运行`verantyx setup`时先用英语，选择语言后引导和选项说明随之切换。首次退出会提供模拟教程。在教程输入`verantyx`可结束练习并进入真正的CLI，`/done`也可结束练习视图。窗口尺寸提示不会阻止继续。
...
| 输入 | 作用 |
|---|---|
| `verantyx new [name]` | 新建Agent对话，保留Owner |
| `verantyx new --owner [name]` | 从外壳新建Owner Cleanroom，先确认 |
| `verantyx cleanroom [name]` | 列出Owner Cleanroom，切换前确认 |
| `verantyx compact` | 由AI压缩带来源的上下文，不删除原文 |
| `/insights` | 查看最近的实施中解释 |
| `L-000001` | 在安全节点就该笔记提问 |
| `/approvals` | 查看候选修改权限 |
| `/queue` | 执行中输入可排队或在下个安全节点应用 |

Owner备忘带时间戳，保存在私有本地SQLite中。界面分页读取，搜索查询已存笔记。原始工作事件留在项目账本中。AI摘要只是带来源的额外视图，不替换原文。这降低日常读取负荷，但不保证完整账本重放或导出的内存恒定。

浏览器体验复现切换输入、引用、备忘、连续对话和示例授权。数据仅留在此标签页，不是CLI数据库。脚本估时与结果不是真实工作或证据。订阅执行、实际压缩、持久会话及文件编辑需要本地CLI；“请求我的AI”单独明确连接。

[当前操作](../../docs/two-pane-interaction.md) · [学习与会话](../../docs/live-learning-and-sessions.md) · [英语演示录制](../../docs/DEMO_RECORDING.md)
