...
モデル番号の入力は選択専用です。両方の選択後に再度番号を入れると、新しい2モデルの選択を始めます。依頼文を送信したときだけ、主Macの40B上限・予備Macの10B上限と実行先を確定してから、一件の承認を求めます。旧画面から数字だけ送られた場合もサーバーで拒否します。

`notify_email`が設定されていれば、一件ごとにMacのMailから承認依頼メールを送ります。メールに両モデル・合計パラメータ数・依頼文・実行先を記載し、このMacの管理ページで許可/拒否を選べます。リンクを開くだけでは承認されません。リンクはMac内のlocalhostに接続するため、同じMacで開いてください。スマートフォンからの承認には対応していません。MacのMailの送信アカウント設定とOSの操作許可が必要で、送信の成否をローカル監査履歴に残します。
